...
classdef eegbrowse < hgsetget & visprops.configurable
    ...
    methods(Static = true)
        ...
        function visData = getBlockDataFromEEG(EEG, dataID, blocksize)
            ...
            [event, startTimes, timeScale] = viscore.blockedEvents.getEEGTimes(EEG);
            visData = viscore.memoryData(EEG.data, dataID, ...
                    'SampleRate', EEG.srate, 'Events', event, ...
                    'ElementLocations', chanlocs, 'BlockSize', blocksize, ...
                    'Epoched', ndims(EEG.data) == 3, ...
                    'BlockTimeScale', timeScale, 'BlockStartTimes', startTimes);
