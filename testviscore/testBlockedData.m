...
fprintf('It should compute the trimmed values correctly\n');
[tMean, tStd, tLow, tHigh] = bData.getTrimValues(10, 0.5:1:99.5);
assertElementsAlmostEqual(tLow, 5);
assertElementsAlmostEqual(tHigh, 95);
assertElementsAlmostEqual(tMean, mean(5.5:1:94.5));
assertElementsAlmostEqual(tStd, std(5.5:1:94.5, 1));
fprintf('It should compute trimmed values when data is not passed\n');
[tMean, tStd, tLow, tHigh] = bData.getTrimValues(10);
assertTrue(~isnan(tMean));
assertTrue(~isnan(tStd));
assertTrue(~isnan(tLow));
assertTrue(~isnan(tHigh));
[tMean, tStd, tLow, tHigh] = bData.getTrimValues(0);
x = bData.getData;
assertElementsAlmostEqual(mean(x(:)), tMean);
assertElementsAlmostEqual(std(x(:), 1), tStd);
assertElementsAlmostEqual(min(x(:)), tLow);
assertElementsAlmostEqual(max(x(:)), tHigh);
