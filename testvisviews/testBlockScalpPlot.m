...
load('SingleFrame.mat');
load('EEG.mat');
values.sFrame = viscore.memoryData(singleFrame, 'EEG', ...
    'SampleRate', EEG.srate, 'ElementLocations', EEG.chanlocs);
...
fprintf('It should produce a plot for a slice consisting of single frame\n');
singleFrameFile = regexprep(which('EEG.mat'), 'EEG.mat$', 'SingleFrame.hdf5');
load('SingleFrame.mat');
load('EEG.mat');
hdf5SFrame = viscore.hdf5Data(singleFrame, 'EEG', singleFrameFile, ...
    'SampleRate', EEG.srate, 'ElementLocations', EEG.chanlocs);
fig5 = figure('Name', 'Slice is single frame');
sm5 = visviews.blockScalpPlot(fig5, [], []);
slice5 = viscore.dataSlice('Slices', {':', ':', ':'}, ...
    'DimNames', {'Channel', 'Sample', 'Window'});
assertTrue(isvalid(sm5));
sm5.plot(hdf5SFrame, values.fun, slice5);
gaps = sm5.getGaps();
sm5.reposition(gaps);
