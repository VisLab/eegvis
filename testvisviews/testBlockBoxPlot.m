...
% Set up the data
load('UnevenGrouping.mat');
testVD6 = viscore.memoryData(unevenGrouping, 'Specific values');
...
load('LargeSTD.mat');
bData8 = viscore.memoryData(largeSTD, 'Random normal large std');
...
% Set up the data
load('UnevenGrouping.mat');
unevenGroupingFile = regexprep(which('EEG.mat'), 'EEG.mat$', 'UnevenGrouping.hdf5');
testVD6 = viscore.hdf5Data(unevenGrouping, 'Specific values', unevenGroupingFile);
fprintf('It should produce a plot for identity slice with uneven grouping\n');
fig6 = figure('Name', 'No grouping to compare with specific values');
bp6 = visviews.blockBoxPlot(fig6, [], []);
assertTrue(isvalid(bp6));
...
fprintf('It should produce a plot for data with large standard deviation\n');
load('LargeSTD.mat');
largeSTDFile = regexprep(which('EEG.mat'), 'EEG.mat$', 'LargeSTD.hdf5');
bData8 = viscore.hdf5Data(largeSTD, 'Random normal large std', largeSTDFile);
fig8 = figure('Name', 'Data with large standard deviation');
bp8 = visviews.blockBoxPlot(fig8, [], []);
assertTrue(isvalid(bp8));
bp8.plot(bData8, values.fun, values.slice);
...
bp1 = visviews.blockBoxPlot(fig1, [], []);
assertTrue(isvalid(bp1));
bp1.plot(values.hdf5Data, values.fun, []);
gaps = bp1.getGaps();
bp1.reposition(gaps);
...
plotSlice = viscore.dataSlice('Slices', {'7:18', ':', '12:15'}, ...
         'DimNames', {'Channel', 'Sample', 'Window'});
bp1.plot(values.hdf5Data, values.fun, plotSlice);
gaps = bp1.getGaps();
bp1.reposition(gaps);
