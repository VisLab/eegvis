...
% function teardown(values)  %#ok<INUSD>
...
sh10.plot(values.bDataSmoothTrim, values.fun, values.slice);
gaps = sh10.getGaps();
...
sh13.plot(values.bDataSmoothSingleWindow, values.fun, slice13);
gaps = sh13.getGaps();
...
sh14.plot(values.bDataSmoothSingleWindow, values.fun, slice14);
gaps = sh14.getGaps();
...
assertTrue(isvalid(sh9));
sh9.HistogramColor = [1, 0, 0];
sh9.plot(values.hdf5DataSmooth, values.fun, values.slice);
gaps = sh9.getGaps();
sh9.reposition(gaps);
...
sh10 = visviews.signalHistogramPlot(fig10, [], []);
assertTrue(isvalid(sh10));
sh10.plot(values.hdf5DataSmoothTrim, values.fun, values.slice);
gaps = sh10.getGaps();
sh10.reposition(gaps);
...
slice11 = viscore.dataSlice('Slices', {':', ':', '2:4'}, ...
    'DimNames', {'Channel', 'Sample', 'Window'}, 'CombineDim', 3);
sh11.plot(values.hdf5DataSmooth, values.fun, slice11);
gaps = sh11.getGaps();
sh11.reposition(gaps);
...
slice13 = viscore.dataSlice('Slices', {':', ':', '4:8'}, ...
    'DimNames', {'Channel', 'Sample', 'Epoch'}, 'CombineDim', 3);
sh13.plot(values.hdf5DataSmoothSingleWindow, values.fun, slice13);
gaps = sh13.getGaps();
sh13.reposition(gaps);
...
slice14 = viscore.dataSlice('Slices', {'30:32', ':', ':'}, ...
    'DimNames', {'Channel', 'Sample', 'Window'}, 'CombineDim', 1);
sh14.plot(values.hdf5DataSmoothSingleWindow, values.fun, slice14);
gaps = sh14.getGaps();
sh14.reposition(gaps);
drawnow
...
% sh1 = visviews.signalHistogramPlot(fig1, [], []);
% assertTrue(isvalid(sh1));
% sh1.plot(values.hdf5AllZerosData, values.fun, values.slice);
% gaps = sh1.getGaps();
% sh1.reposition(gaps);
% 
% % Data zeros, function NaN
...
% sh2 = visviews.signalHistogramPlot(fig2, [], []);
% assertTrue(isvalid(sh2));
% sh2.plot(values.hdf5AllZerosData, [], values.slice);
% gaps = sh2.getGaps();
% sh2.reposition(gaps);
% 
% % Data NaN
...
% sh3 = visviews.signalHistogramPlot(fig3, [], []);
% assertTrue(isvalid(sh3));
% sh3.plot(values.hdf5NaNData, values.fun, values.slice);
% gaps = sh3.getGaps();
% sh3.reposition(gaps);
% 
% % Data slice empty
...
% sh4 = visviews.signalHistogramPlot(fig4, [], []);
% assertTrue(isvalid(sh4));
% sh4.plot(values.hdf5EmptySliceData, values.fun, slice4);
% gaps = sh4.getGaps();
% sh4.reposition(gaps);
% drawnow
% if values.deleteFigures
